...
(assert (= (val n_service/id) s_129.27.142.49))
(assert (not (miss n_service/id)))
(assert (not (err n_service/id)))

(assert cns_anon_proxy_https_permit)(echo " --> Check MAY property... (holds if the following check is sat)")
