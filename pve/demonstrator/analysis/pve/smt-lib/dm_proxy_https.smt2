...
;################### STRING DECLARATIONs #######################
 (declare-datatypes () ((String s_demo-app_dm s_129.27.142.49 s_GET  s_AdditionalStringValue )))
...
(declare-const const_demo-app_dm (TValue String))
(assert (= (val const_demo-app_dm) s_demo-app_dm))
(assert (not (miss const_demo-app_dm))) 
(assert (not (err const_demo-app_dm)))
...
;################### START CONSTRAINT RULE mask #######################
;##### Rule Target
(define-fun cns_target_mask () (TValue Bool)
	(FAnd (equalString n_response/method const_GET) (equalInt n_response/response - code const_200))
)
;##### Rule Obligations
(define-fun cns_obl_permit_mask ()  Bool
	 (and true))
(define-fun cns_obl_deny_mask ()  Bool true )
;##### Rule Constraints
;PERMIT
(define-fun cns_mask_permit () Bool
(and (isTrue cns_target_mask) cns_obl_permit_mask)
)
;DENY
(define-fun cns_mask_deny () Bool
 false 
)
;NOT APP
(define-fun cns_mask_notApp () Bool
	(or (isFalse cns_target_mask) (miss cns_target_mask))
)
;INDET
(define-fun cns_mask_indet () Bool
	(or 
		(not
			(or  
				(isBool cns_target_mask)
				(miss cns_target_mask)
			)
		)
		(and 
			(isTrue cns_target_mask)
			(not cns_obl_permit_mask)
		)
	)
)
;################### END CONSTRAINT RULE mask #########################
...
;################################ TOP-LEVEL POLICY dm_proxy_https CONSTRAINTs ###########################
;##### Policy Target
(define-fun cns_target_dm_proxy_https () (TValue Bool)
	(FOr (FAnd (equalString n_service/id const_129.27.142.49) (equalString n_response/path const_demo-app_dm)) (FAnd (equalString n_service/id const_129.27.142.49) (equalString n_request/path const_demo-app_dm)))
)
;##### Policy Obligations
(define-fun cns_obl_permit_dm_proxy_https ()  Bool
true
)
(define-fun cns_obl_deny_dm_proxy_https ()  Bool
true
)
;##### Policy Combining Algorithm
(define-fun cns_dm_proxy_https_cmb_final_permit () Bool
	 (or 
		 (and cns_mask_permit cns_mask_passthrough_permit)
		 (and cns_mask_permit cns_mask_passthrough_notApp)
		 (and cns_mask_notApp cns_mask_passthrough_permit)
	 )
)

(define-fun cns_dm_proxy_https_cmb_final_deny () Bool
	 (or cns_mask_deny cns_mask_passthrough_deny)
)

(define-fun cns_dm_proxy_https_cmb_final_notApp () Bool
	 (and cns_mask_notApp cns_mask_passthrough_notApp)
)

(define-fun cns_dm_proxy_https_cmb_final_indet () Bool
	 (or 
		 (and cns_mask_indet (not cns_mask_passthrough_deny))
		 (and (not cns_mask_deny) cns_mask_passthrough_indet)
	 )
)
...
;PERMIT
(define-fun cns_dm_proxy_https_permit () Bool
	(and 
		(isTrue cns_target_dm_proxy_https)
		cns_dm_proxy_https_cmb_final_permit
		cns_obl_permit_dm_proxy_https
	)
)
;DENY
(define-fun cns_dm_proxy_https_deny () Bool
	(and 
		(isTrue cns_target_dm_proxy_https)
		cns_dm_proxy_https_cmb_final_deny
		cns_obl_deny_dm_proxy_https
	)
)
;NOT APP
(define-fun cns_dm_proxy_https_notApp () Bool
	(or
		(or (isFalse cns_target_dm_proxy_https) (miss cns_target_dm_proxy_https))
		(and (isTrue cns_target_dm_proxy_https) cns_dm_proxy_https_cmb_final_notApp)
	)
)
;INDET
(define-fun cns_dm_proxy_https_indet () Bool
	(or 
		(not
			(or  
				(isBool cns_target_dm_proxy_https)
				(miss cns_target_dm_proxy_https)
			)
		)
		(and (isTrue cns_target_dm_proxy_https) cns_dm_proxy_https_cmb_final_indet)
		(and 
			(isTrue cns_target_dm_proxy_https)
			cns_dm_proxy_https_cmb_final_permit
			(not cns_obl_permit_dm_proxy_https)
		)
		(and 
			(isTrue cns_target_dm_proxy_https)
			cns_dm_proxy_https_cmb_final_deny
			(not cns_obl_deny_dm_proxy_https)
		)
	)
)
;################### END TOP-LEVEL POLICY dm_proxy_https CONSTRAINTs #########################
